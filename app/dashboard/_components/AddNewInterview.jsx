...
function AddNewInterview() {
  ...
  const handleResumeUpload = async (e) => {
    ...
    if (
      ...
      setResumeError(
        "Please upload a valid PDF or DOCX file. Other file types are not supported."
      );
      setResumeFile(null);
      setResumeText("");
      return;
    ...
    const invalidExt =
      /\.(exe|jpg|jpeg|png|gif|bmp|txt|js|py|zip|rar|mp3|mp4|avi|mov|html|css|json|csv|svg)$/i;
    if (invalidExt.test(file.name)) {
      setResumeError(
        "This does not look like a resume file. Please upload your actual resume in PDF or DOCX format."
      );
      setResumeFile(null);
      setResumeText("");
      return;
    ...
    try {
      if (file.type === "application/pdf") {
        ...
        reader.onload = async function () {
          ...
          setResumeText(result.value.slice(0, 5000));
        };
        reader.readAsArrayBuffer(file);
      }
    } catch (err) {
...
The next ${
      questionCount - 1
    } objects should each have "question" and "answer" fields and be based on the technical skills and job description provided (not on the resume, unless relevant).


${
  includeDSA
    ? `Add one final object with "dsaQuestion" and "dsaAnswer" fields. The DSA question should be specific, include example input/output test cases, and provide a detailed theoretical explanation of the solution.`
    : ""
}
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              <span className="block text-gray-500 mt-2">
                Optionally upload your resume (PDF or DOCX). Some questions will
                be tailored to your resume!
              </span>
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={onSubmit}>
            <div>
              ...
              <div className="my-3">
                ...
                <Textarea
                  placeholder="Ex. React, Angular, Spring Boot etc."
                  required
              ...
              <div className="my-3">
                ...
                <label
                  ...
                  className="flex items-center gap-3 px-4 py-2 border-2 border-dashed border-blue-400 rounded-lg cursor-pointer bg-blue-50 hover:bg-blue-100 transition"
                >
                ...
                {resumeError && (
                  <div className="text-red-600 text-sm mt-1">{resumeError}</div>
                )}
